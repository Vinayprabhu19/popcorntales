...
const BottomNavigationAction = lazy(() =>
...
class Home extends Component {
  ...
  onPageChanged(e, v) {
    ...
    var start = 18 * (v - 1);
    var len =
      this.state.reviews.length > 18 * v ? 18 * v : this.state.reviews.length;
  ...
  onSearch(e) {
    ...
    const filteredReviews = this.state.allReviews.filter((review) =>
    ...
    return (
      <>
        <Helmet>
          ...
          <meta
            name="description"
            content="Find the latest movies reviews from various Indian and Foreign languages across platforms"
          />
            ...
            <div className={this.state.loading ? "hidden" : "App"}>
              ...
              <Suspense>
                ...
                >
                  ...
                  />
                  <Tooltip title="Sort">
                    <Button className="iconBtn" onClick={this.openSort}>
                      <SortIcon style={{ fill: "purple" }} />
                    </Button>
                  </Tooltip>
                  <Tooltip title="Filter">
                    <Button className="iconBtn" onClick={this.openFilter}>
                      <FilterListIcon style={{ fill: "purple" }} />{" "}
                    </Button>
                  </Tooltip>
                  <Sort
                ...
                >
                  <Pagination
                    ...
                    onChange={(event, value) =>
                      this.onPageChanged(event, value)
  ...
  processImageData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      data[i].titleImage =
        data[i].titleImage + "&width=" + width + "&height=" + height;
    }
    return data;
  }
  handleFilterClose(data) {
    ...
    let reviews = this.state.allReviews.filter((movie) => {
      ...
      const ratingCheck =
        movie.rating >= rating[0] && movie.rating <= rating[1];
  ...
  handleSortClose(data) {
    ...
    if (
      data.field === this.state.sorter.field &&
      data.sortType === this.state.sorter.sortType
    ) {
    ...
    if (data.field === "timeStamp") {
      ...
      reviews.sort((a, b) => {
        ...
        return data.sortType === "Descending"
          ? valueB - valueA
          : valueA - valueB;
      });
    }
