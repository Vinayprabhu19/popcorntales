...
const ColumnChart = React.lazy(() =>
  import("react-chartkick").then((module) => ({
...
class Analytics extends Component {
  ...
  getFilteredData(result) {
    for (var i = 0; i < result.length; i++) {
      if (this.state.filterData.languages.includes(result[i].language))
        continue;
  ...
  handleFilterClose(data) {
    ...
    if (data.filter.genre != "All") {
      reviews = reviews.filter(function (r) {
        return r.genre.includes(data.filter.genre);
      });
    }
    reviews = reviews.filter(function (r) {
      return (
        r.rating >= data.filter.rating[0] && r.rating <= data.filter.rating[1]
      );
    });
      ...
      <div>
          ...
          >
            <div style={{ width: "100%" }}>
              <Typography
                ...
                style={{ color: "purple" }}
              >
                Total Movies : {this.state.totalMovies}
              </Typography>
            </div>
          </AccordionSummary>
          <AccordionDetails>
        ...
        <Paper elevation={10} id="filterBar">
          <Tooltip title="Filter">
            <Button className="iconBtn" onClick={this.openFilter}>
              <FilterListIcon fontSize={"large"} style={{ fill: "purple" }} />{" "}
            </Button>
          </Tooltip>
        </Paper>
        <Filter
        ...
        <div id="section">
          <div id="centerSection">
              ...
              >
                <div style={{ width: "100%" }}>
                  <Typography
                    ...
                    style={{ color: "purple" }}
                  >
                    High Rated : {this.state.bestMovie.title} -{" "}
                    {this.state.bestMovie.rating}
                  </Typography>
                </div>
              </AccordionSummary>
              <AccordionDetails>
              ...
              >
                <div style={{ width: "100%" }}>
                  <Typography
                    ...
                    style={{ color: "purple" }}
                  >
                    Low Rated : {this.state.worstMovie.title} -{" "}
                  {this.state.worstMovie.rating}
                  </Typography>
                  </div>
              </AccordionSummary>
              <AccordionDetails>
            ...
            <Grid container justifyContent="center">
            <Typography variant="subtitle1" fontWeight={"bold"} style={{margin:"10px"}} >Movies in filter - {this.state.reviews.length}</Typography>
        ...
        <footer>
          <Grid container justifyContent="center" id="footerText">
            <h6>
              ...
              <a href="mailto:popcorntales19@gmail.com">
                {" "}
                <MailIcon />
              </a>
            </h6>
  ...
  getRatingsMode(result) {
    ...
    for (i = 0; i < ratings.length; i++) {
      ...
      ratingsCount.push([ratings[i], count]);
    }
    return ratingsCount;
  }
  ...
  getMovieCountByYear(result) {
    ...
    for (i = 0; i < year.length; i++) {
      const count = result.filter(
        (obj) => new Date(obj.timeStamp).getFullYear() === year[i]
      ).length;
  ...
  getratingCountByLanguage(result) {
    ...
    for (i = 0; i < languages.length; i++) {
      var filteredByLanguage = result.filter(
        (obj) => obj.language === languages[i]
      );
      var min = 999;
      var max = -1;
      ...
      for (var j = 0; j < filteredByLanguage.length; j++) {
        if (filteredByLanguage[j].rating < min)
          min = filteredByLanguage[j].rating;
        if (filteredByLanguage[j].rating > max)
          max = filteredByLanguage[j].rating;
  ...
  getRatingCountByCategory(result) {
    ...
    for (i = 0; i < catergories.length; i++) {
      var filteredByCategory = result.filter((obj) =>
        obj.genre.includes(catergories[i])
      );
      var min = 999;
      var max = -1;
      var sum = 0;
      for (var j = 0; j < filteredByCategory.length; j++) {
        if (filteredByCategory[j].rating < min)
          min = filteredByCategory[j].rating;
        if (filteredByCategory[j].rating > max)
          max = filteredByCategory[j].rating;
